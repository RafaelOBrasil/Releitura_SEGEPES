...
import { Props, Instituicoes } from "@/types/listas";

const itemsPerPage = 10;

export default function ListInstituicoes({ instituicoes }: Props) {
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState("");

  const instituicaoKeys = instituicoes?.[0]
    ? Object.keys(instituicoes[0]).filter((key) => key !== "id")
    : [];

  const filteredInstituicoes = (instituicoes ?? []).filter((instituicao) =>
    (instituicao.nome ?? "").toLowerCase().includes(search.toLowerCase()) ||
    (instituicao.endereco ?? "").toLowerCase().includes(search.toLowerCase()) ||
    (instituicao.CNPJ ?? "").includes(search)
  );

  const totalPages = Math.ceil(filteredInstituicoes.length / itemsPerPage);
  const start = (page - 1) * itemsPerPage;
  const end = start + itemsPerPage;
  const paginatedInstituicoes = filteredInstituicoes.slice(start, end);
  ...
  return (
    <div className="space-y-4 m-6">
      ...
      <Table className="border border-black rounded-2xl overflow-hidden">
        ...
        <TableBody>
          {paginatedInstituicoes.map((instituicao) => (
            <TableRow key={instituicao.id}>
              {instituicaoKeys.map((key, idx) => (
                <TableCell key={idx}>
                  {instituicao[key as keyof Instituicoes]}
                </TableCell>
              ))}
